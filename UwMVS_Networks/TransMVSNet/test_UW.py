...
parser.add_argument('--coef_path', default='/home/gdyang/Downloads/Data/Underwater/UW_Types', help='underwater coefficients path') # for underwater dataset
...
def save_depth(testlist, outdir, testpath):

    for scene in testlist:
        save_scene_depth([scene], outdir, testpath)

# run CasMVS model to save depth maps and confidence maps
def save_scene_depth(testlist, outdir, testpath):
    # dataset, dataloader
    MVSDataset = find_dataset_def(args.dataset)
    test_dataset = MVSDataset(testpath, testlist, "test", args.num_view, args.numdepth, Interval_Scale,
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence, conf_1, conf_2 in zip(filenames, cams, imgs, \
                ...
                depth_color.save(os.path.join(outdir, filename.format('depth_est', '.png')))
...
def pcd_filter_worker(scan, outdir, testpath):
    ...
    scan_folder = os.path.join(outdir, scan)
    out_folder = os.path.join(outdir, scan)
    filter_depth(pair_folder, scan_folder, out_folder, os.path.join(outdir, save_name))
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.coef_path, 'UW_Scene_Type.txt')) as f:
        ...
        for scene in scenes:
            print("***************** MVS for {} Scene ******************".format(scene))

            outdir = os.path.join(args.outdir, "{}".format(scene))
            testpath = os.path.join(args.testpath, "{}".format(scene))
            ...
            if args.testlist != "all":
                ...
                testlist = [e for e in os.listdir(testpath) if os.path.isdir(os.path.join(testpath, e))] \
                    if not args.testpath_single_scene else [os.path.basename(args.testpath_single_scene)]

            # step1. save all the depth maps and the masks in outputs directory
            save_depth(testlist, outdir, testpath)

            # step2. filter saved depth maps with photometric confidence maps and geometric constraints
            if args.filter_method == "normal":
                pcd_filter(testlist, args.num_worker)
            elif args.filter_method == "gipuma":
                gipuma_filter(testlist, outdir, args.prob_threshold, args.disp_threshold, args.num_consistent,
