        ...
        for batch, data in enumerate(self.train_loader):
            ...
            if is_main_process():
                ...
                pbar.update(batch, LR=self.optimizer.param_groups[0]['lr'],
                            Loss="{}|{}".format(scalar_outputs["loss"], avg_scalars.avg_data["loss"]),
                            Abs="{}|{}".format(scalar_outputs["abs_depth_error"], avg_scalars.avg_data["abs_depth_error"]),
                            Th2="{}|{}".format(scalar_outputs["thres2mm_error"], avg_scalars.avg_data["thres2mm_error"]),
                            Th4="{}|{}".format(scalar_outputs["thres4mm_error"], avg_scalars.avg_data["thres4mm_error"]),
                            Th8="{}|{}".format(scalar_outputs["thres8mm_error"], avg_scalars.avg_data["thres8mm_error"]))
    ...
    def validate(self, epoch=0):
        ...
        if is_main_process():
            pwidgets = [progressbar.Percentage(), " ", progressbar.Counter(format='%(value)02d/%(max_value)d'), " ", progressbar.Bar(), " ",
                        ...
                        progressbar.Variable('Abs', width=1), ",",
        ...
        if is_main_process():
            pbar.finish()
            print("final", avg_scalars.avg_data)
        ...
        for scene in testlist:
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                for filename, cam, img, depth_est, depth2, depth1, photometric_confidence, pc2, pc1 \
                    ...
                    depth_color.save(os.path.join(self.args.outdir, filename.format('depth_est', '.png')))
                    #print(filename) # scan9/{}/00000000{}
                    #print(filename.format('depth_est', '.png')) # scan9/depth_est/00000000.png
                    depth_color.save(os.path.join(self.args.outdir, filename.format('depth_est', '.png')))
                    # save confidence maps
                    #print(photometric_confidence.min(), photometric_confidence.max())
                    save_pfm(confidence_filename, photometric_confidence)
                    save_pfm(confidence_filename2, pc2)
                    save_pfm(confidence_filename1, pc1)
                    conf_color = visualize_conf(photometric_confidence)
                    conf_color.save(os.path.join(self.args.outdir, filename.format('confidence', '.png')))
                    # save cams, img
            ...
            torch.cuda.empty_cache()
            end_time_for_total = time.time()
            total_time_for_depth = end_time_for_total - start_time_for_total
            print("Total Depth Estimation Time: {}".format(total_time_for_depth))
            print("Average Inference Time per Image: {}".format(total_time_for_depth / len(TestImgLoader)))
...
# step2. filter saved depth maps with photometric confidence maps and geometric constraints
        if self.args.filter_method == "pcd":
            ...
            end_time_gipuma = time.time()
            #print("Total Depth Fusion Time: {}".format(end_time_gipuma - start_time_gipuma))
